...
public:
	...
	int select_window(std::vector<Mainclass::Gun>& gun_data,std::unique_ptr<H, T>& settings) {
	...
	template<class Y, class D>
	void set_draw(Mainclass::Chara& chara, std::unique_ptr<Y, D>& scoreparts,const bool& usegun, const int& sel_gun) {
		//
		...
		bufScreen.SetDraw_Screen();
		{
			...
			//開始
			{
				...
				if (!scoreparts->end_f) {
					if (!scoreparts->start_f) {
						...
						if (scoreparts->ready_f) {
							font_big->DrawStringFormat(
								...
								yp + y_r(18, disp_y), GetColor(255, 0, 0), "%d:%05.2f", 0, scoreparts->readytimer);
					...
					easing_set(&ready_yp, float(disp_y / 8 + y_r(18, disp_y)), 0.95f, fps);
					if (!c_end_f) {
			...
			//モードその他
			{
				...
				if (time_tmp >= 0.25f) {
					if (chara.ammo_cnt == 0) {
						font->DrawString(xp - font->GetDrawWidth("EMPTY") / 2, yp, "EMPTY", GetColor(255, 0, 0)); yp += y_r(18, disp_y);
					}
				}
				time_tmp += 1.f / fps;
				if (time_tmp >= 0.5f) {
